...
### v1.0.62 — Chacun son app
La journée avait pris un tournant : l'utilisateur devait pouvoir choisir. Mais une liberté éparpillée devient vite un labyrinthe, surtout pour le collègue qui découvre l'app. Tout ce qui touche à l'apparence est donc rassemblé dans un seul écran, **🎨 Personnaliser**, rangé par zone, avec un aperçu qui suit chaque réglage et un bouton pour tout remettre à l'origine.

Dix idées y sont entrées d'un coup — « j'aime bien tout ». La taille du texte, d'abord, et avec elle une vraie question : pouvait-on zoomer sur toute l'app avec deux doigts ? Le zoom d'une page web grossit l'image sans la réorganiser ; entre deux soins, un bouton sorti de l'écran, c'est une perte de temps. D'où un **zoom intelligent** qui agrandit l'app en la recomposant — et, à sa demande, le choix du zoom classique pour qui le préfère, ou d'aucun zoom pour qui zoome par accident.

Puis les constantes qu'on n'utilise pas et qu'on peut masquer (jamais au point de cacher une valeur hors seuil), le contenu des cartes (les signaux de vigilance ne se décochent pas), la barre d'outils à sa main, un thème de jour et un thème de nuit, une relève signée, l'écran d'ouverture, et le partage de ses réglages entre collègues — une liste fermée, qui ne peut rien emporter d'autre que de l'affichage.
