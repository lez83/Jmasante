...
| `_test_signature` | Injection de la signature dans build.gradle, workflow de release |
| `_test_histo` | Historique par mois, filtres, suppression qui préserve les constantes |
| `_test_liens` | Reconnaissance dans un PDF, documents rattachés, glisser-déposer |
| `_test_seq_nav` | Navigation du déroulé : formulaire vide, retour, Quitter, pas de doublon |
| `_test_fusion` | Ordre conservé à la fusion, sélection, ordre transmis par synchro |
| `_test_tension` | Constantes par jour, tension en mm Hg, brouillon effacé à la validation |
| `_test_ouvrir` | Ouverture d'un fichier reçu : manifeste Android, lecture, verrouillage |
| `_test_contacts` | Reprise des contacts, listes, prescripteur, export, catalogues |
| `_test_autonomie` | Notation du catalogue, sous-rubriques, relève et Sélection |
...
- `_test_affichages.js` : tuiles de constantes, cartes à bande, légende, code à 6 chiffres, code de secours, ancien code à 4 chiffres.
- `_test_echanges.js` : quatre appareils simulés — mot de passe, appairage sécurisé (ECDH) et rapide, fichier à plusieurs serrures, réception, écran d'envoi, oubli d'un collègue.

⚠️ `_test_j1creneau.js` affiche « à revoir » depuis avant la v1.0.57 (même résultat sur la v1.0.56) : à examiner à part.

- `_test_cle.js` (v1.0.59) : lecture des données en ancienne clé, migration en arrière-plan vers la clé v2, redémarrage, sel stable.
- `_test_couleurs.js` (v1.0.60) : palettes, retouche, garde-fou, motifs et formes, palette Daltonisme, retour à l'origine.
- `_test_perso.js` (v1.0.62) : écran Personnaliser, texte et zoom, constantes masquées/forcées, contenu des cartes, barre d'outils, réinitialisation, thème jour/nuit, relève par défaut, partage des réglages, ouverture.
