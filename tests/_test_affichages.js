...
function run(){
  ...
  w.eval(appjs + '\n;window.__B = { sheetPatient, sheetAppPanel, getS:()=>S, render, applyTheme, hashSecours, genCodeSecours, normSecours, sensConst, derniereConst, showLock, pinKey, codeOublie };');
  ...
  (async()=>{
    ...
    S.cardStyle='classique'; B.applyTheme(); B.sheetAppPanel(); await wait(100);
    ok(!!d.querySelector('#dotpick'),'choix de taille visible en mode Pastille');
    ...
    S.cardStyle='bande'; B.applyTheme(); B.sheetAppPanel(); await wait(100);
    ok(!d.querySelector('#dotpick'),'choix de taille masqué en mode Bande');
