...
function run(){
  ...
  (async()=>{
    ...
    const champs=['f-nom','f-prenom','f-dob','f-nir','f-addr','f-appar',
                  'f-infos','f-plan','f-newplan','f-addplan','f-tours','f-save','f-cancel'];
    const manque=champs.filter(i=>!q('#'+i));
    ok(champs.length+' champs présents'+(manque.length?' — manque '+manque.join(','):''), !manque.length);
    // Médecins et entourage en listes ; pharmacie et cabinet en champs fixes
    ok('Contacts (listes + pharmacie/cabinet)', !!q('[data-lc="med"]') && !!q('[data-lc="ent"]') && qa('.f-contact-name').length>=2);
    ok('Seuils personnalisés', qa('.f-th').length>=6);
    ok('Valeurs chargées', q('#f-nom')?.value==='Démo-Leroy' && !!q('#f-nir')?.value && !!q('#f-appar')?.value);
    ok('Contact médecin chargé', q('[data-lc="med"] [data-lcnom]')?.value==='Dr Démo');
    ...
    ok('PERSISTANCE : contacts', q('[data-lc="med"] [data-lcnom]')?.value==='Dr Démo');
