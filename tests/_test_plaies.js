...
function run(){
  ...
  w.eval(appjs + '\n;window.__P = { PLAIE_TYPES, PLAIE_REGIONS, PLAIE_ZONES_SVG, plaieNom, plaieJours, plaiePhotos, plaiesOuvertes, docPlaieOuverte, plaieTexteReleve, plaieToutesZones, plaieRegionDe, plaieSilhouette, sheetPlaies, getP };');
  ...
  (async()=>{
    ...
    console.log('\n═══ ZONES CLIQUABLES ═══');
    ['dos','face'].forEach(v => {
      const zs = P.PLAIE_ZONES_SVG[v];
      const inconnues = zs.filter(x => !z.includes(x.n)).map(x=>x.n);
      console.log(`  ${v.padEnd(5)} ${zs.length} zones`, inconnues.length ? '⚠ hors liste : '+inconnues : '✓ toutes dans la liste');
    });
