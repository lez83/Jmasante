...
const PLAN=["Toilette","TA / Pouls","Pansement jambe gauche","Conscience / Fonctions cognitives","Bas de contention"];
// ⚠️ Ce soin est coché MATIN dans le plan, mais fait aussi le soir :
// il ne doit PAS ressortir en « soins supplémentaires ».
const PLAN_SLOTS={"Conscience / Fonctions cognitives":{matin:true,soir:false}};
const visits=[];
for (let d=11; d>=0; d--){
  ["matin","soir"].forEach((sl,k)=>{
    ...
    v.soins.push("Conscience / Fonctions cognitives");   // fait matin ET soir
    if (d===7 && sl==="matin") v.soinNotes={"Pansement jambe gauche":"plaie propre, bourgeonnement"};
    if (d===5 && sl==="soir") v.note="nuit agitée d'après la fille";
    visits.push(v);
  });
}
const state={version:1,theme:"hopital",retention:12,pin:null,lastGreeting:iso,slotsEnabled:true,
  ...
  patients:[{id:"p1",nom:"Démo-Martin",prenom:"Renée",dob:"1938-04-12",genre:"F",address:"",
    ...
    plaies:[],visits,planSlots:PLAN_SLOTS,thresholds:{taSysMax:15,taDiaMax:9}}],
...
function run(){
  ...
  setTimeout(()=>{
    const R=w.__R;
    const txt = R.buildReleve({ start:j(11), end:j(0), mode:"all", withRaps:false });
    ...
    console.log('  pas de faux « supplémentaires » :',
      !/Soins suppl.*Conscience/.test(txt) ? '✓' : '⚠ SOIN DU PLAN CLASSÉ HORS PLAN');
    console.log('\n  longueur :', l.filter(x=>x.trim()).length, 'lignes');
    console.log('\n═══ DOCUMENTS À JOINDRE : UN PATIENT À LA FOIS ═══');
const sha = fs.readFileSync('../jmsante/www/js/share.js','utf-8');
const cssa = fs.readFileSync('../jmsante/www/css/app.css','utf-8');
const v = (l, c) => { console.log('  ' + (c?'✓':'⚠') + ' ' + l); if(!c) ko.push(l); };
// ⚠️ Tous les documents de tous les patients dépliés : liste interminable.
v('groupes repliés par défaut', /doc-grp-b" \$\{joints \? "" : "hidden"\}/.test(sha));
v('un bouton par patient', /data-grp="\$\{gi\}"/.test(sha));
v('compte des documents sur l en-tête', /doc-n">📎 \$\{g\.items\.length\}/.test(sha));
v('badge « joints » visible même replié', /doc-badge/.test(sha) && /joint" \+ \(n>1\?"s":""\)/.test(sha));
v('ouvrir un patient referme les autres', /\$\$\("#attlist \.doc-grp-b"\)\.forEach\(c => c\.setAttribute\("hidden"/.test(sha));
v('le patient qui a déjà une pièce cochée est ouvert', /\$\{joints \? "" : "hidden"\}/.test(sha));
v('style du pliage', /\.doc-grp\.ouv\{/.test(cssa) && /\.doc-badge\{/.test(cssa));
