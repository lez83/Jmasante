...
console.log('\n═══ EXIGENCES DU PLAY STORE ═══');
const ini = fs.readFileSync('../jmsante/www/js/init.js','utf-8');
// ⚠️ Depuis le 31/08/2026 Google refuse toute soumission sous l'API 36.
ck('compileSdk et targetSdk portés à 36', /compileSdkVersion = 36/.test(pc) && /targetSdkVersion = 36/.test(pc));
// ⚠️ Le Play Store n'accepte plus d'APK pour une nouvelle application.
ck('un bundle .aab est produit', /bundleRelease/.test(wf) && /app-release\.aab/.test(wf));
ck('l APK reste produit pour l installation directe', /assembleRelease bundleRelease/.test(wf));
ck('les deux livrables sont publiés', (wf.match(/app-release\.aab/g)||[]).length >= 2);
ck('avertissement au premier lancement', /avertissementPremierLancement/.test(ini));
ck('il dit ce que l app N EST PAS', /pas un dispositif médical/.test(ini) && /n'interprète pas/.test(ini));
ck('affiché une seule fois', /if \(S\.avertLu\) return/.test(ini) && /S\.avertLu = true/.test(ini));
