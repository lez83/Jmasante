...
function _bindUnForm(p, f){
  ...
  qEl(f, "data-cancel").onclick = async () => {
    ...
    if (garde){ delete S.drafts[p.id]; save(); }
...
<p class="small" style="margin-bottom:8px">Pour t'en défaire : rouvre la carte, puis <b>✓ Valider le passage</b> pour l'enregistrer, ou <b>Annuler</b> pour l'effacer — dans les deux cas l'étiquette disparaît.</p>
...
function sheetPatient(p){
  ...
  if (addInfo) addInfo.onclick = e => {
    ...
    pickInfoType(null, t => {
      const creer = rub => {
        infos.push({ id:uid(), type:t||"acces", ...(rub?{rub}:{}), txt:"", show:(t==="acces"||t==="vigilance") });
        drawInfos();
...
function sheetAnnuaire(p){
  ...
  openSheet(`
    ...
    <div class="rap" style="align-items:center">
      ...
      ${x.tel?`<a href="tel:${esc(x.tel)}" class="btn btn-primary" style="padding:8px 16px;text-decoration:none;border-radius:12px">📞 Appeler</a>`:""}
    </div>`).join("")}
...
function sheetSelect(start, end, withRaps, layout, tour, anon){
  ...
  $("#sel-gen").onclick = () => {
    ...
    pool.forEach(p => {
      ...
      const st = PS[p.id];
      pOpts[p.id] = { consts:st.consts, notes:st.notes, bilans:st.bilans, raps:st.raps, docs:st.docs, infos:st.infos };
...
function consommerNotes(){
  let n = 0;
  (S.patients||[]).forEach(p => { if (p.noteReleve){ delete p.noteReleve; n++; } });
  ...
  const editLigne = (tid) => {
    ...
    const drawEdit = () => {
      ...
      <div class="rowbox bl" style="display:block;margin-bottom:12px">
        ...
        </div>${specs.length ? `<div class="rowlab vi"><span>Prescripteur</span><i></i><em>facultatif</em></div>
      <div class="rowbox vi" style="display:block;margin-bottom:12px">
        <div class="chips">
        ...
        </div></div>` : ""}<div class="field"><span class="lab">Remarques <span style="text-transform:none;letter-spacing:0;color:var(--faint)">(facultatif)</span></span>
  ...
  openSheet(`
    ...
    ${sect(RECUEIL_SECTIONS[5], `
      ...
      <div class="rowb" style="align-items:center;margin-top:8px"><div class="rec-lab" style="flex:1;margin:0">MALADIES SUIVIES, ANTÉCÉDENTS</div>
        <button class="chip sm rc-cat" data-rccatx="atcd">📚 Catalogue</button></div>
...
const PLAIE_REGIONS = [
  { cle:"tete", lbl:"Tête & visage", zones:[
    { n:"Cuir chevelu",         vue:"dos"  },
    { n:"Crâne (vertex)",       vue:"dos"  },
    { n:"Occiput",              vue:"dos"  },
    { n:"Nuque",                vue:"dos"  },
    { n:"Front",                vue:"face" },
    { n:"Tempe gauche",         vue:"face" },
    { n:"Tempe droite",         vue:"face" },
    { n:"Arcade sourcilière gauche", vue:"face" },
    { n:"Arcade sourcilière droite", vue:"face" },
    { n:"Paupière gauche",      vue:"face" },
    { n:"Paupière droite",      vue:"face" },
    { n:"Œil gauche",           vue:"face" },
    { n:"Œil droit",            vue:"face" },
    { n:"Nez",                  vue:"face" },
    { n:"Aile du nez",          vue:"face" },
    { n:"Pommette gauche",      vue:"face" },
    { n:"Pommette droite",      vue:"face" },
    { n:"Joue gauche",          vue:"face" },
    { n:"Joue droite",          vue:"face" },
    { n:"Lèvre supérieure",     vue:"face" },
    { n:"Lèvre inférieure",     vue:"face" },
    { n:"Commissure des lèvres", vue:"face" },
    { n:"Menton",               vue:"face" },
    { n:"Mâchoire gauche",      vue:"face" },
    { n:"Mâchoire droite",      vue:"face" },
    { n:"Oreille gauche",       vue:"dos"  },
    { n:"Oreille droite",       vue:"dos"  },
    { n:"Derrière l'oreille gauche", vue:"dos" },
    { n:"Derrière l'oreille droite", vue:"dos" },
    { n:"Cou",                  vue:"face" },
    { n:"Gorge",                vue:"face" },
  ]},
  { cle:"tronc", lbl:"Tronc", zones:[
...
function plaieFil(p, pl){
  ...
  (pl.suivi||[]).forEach(n => ev.push({ d:n.date, t:"note", txt:n.txt, mesures:n.mesures, stade:n.stade, id:n.id }));
...
function plaieRegionsSVG(genre, vue){
  ...
  // ── Tête ── (le zoom ⛶ les rend confortables au doigt)
  if (dos){
    add("Crâne (vertex)",  `M${C-6} 5 q6 -3 12 0 l1 5 q-7 2 -14 0 z`);
    add("Occiput",         `M${C-7} 11 q7 2 14 0 l-1 6 q-6 2 -12 0 z`);
    add("Oreille gauche",  `M${C-g.tete-1} 11 q-3 2 -2 6 q3 1 4 -1 z`);
    add("Oreille droite",  `M${C+g.tete+1} 11 q3 2 2 6 q-3 1 -4 -1 z`);
    add("Nuque",           `M${C-4} 18 q4 2 8 0 l0 4 q-4 1 -8 0 z`);
  } else {
    add("Front",           `M${C-6} 6 q6 -3 12 0 l0 5 q-6 2 -12 0 z`);
    add("Œil gauche",      `M${C-6} 12 q3 -1 4 0 q-1 3 -4 2 z`);
    add("Œil droit",       `M${C+2} 12 q3 -1 4 0 q-1 3 -4 2 z`);
    add("Nez",             `M${C-1.6} 13 q1.6 -1 3.2 0 l0.6 4 q-2.2 1 -4.4 0 z`);
    add("Joue gauche",     `M${C-7} 15 q3 0 4 2 q-2 3 -5 2 z`);
    add("Joue droite",     `M${C+3} 15 q3 0 4 2 q-2 3 -5 2 z`);
    add("Menton",          `M${C-2.6} 19 q2.6 1 5.2 0 l0 3 q-2.6 1 -5.2 0 z`);
    add("Cou",             `M${C-4} 22 h8 v4 h-8 z`);
  }
  // ── Ancien repère (conservé) ──
...
function sheetPlaieFil(pid, plid){
  ...
  openSheet(`
    ...
    <div class="fil">
      ${ev.map(e => `
        <div class="fil-e${e.t==="note"?" cliq":""}"${e.t==="note"?` data-filnote="${e.id}"`:""}>
          ...
          <div class="fil-c">
            ...
            ${e.t==="note" ? `<div class="fil-s">appuie pour modifier</div>` : ""}
          </div>
        </div>`).join("")}
    </div>
  ...
  $$("#sheet [data-filnote]").forEach(b => b.onclick = () => plaieNoteEditer(pid, plid, b.dataset.filnote, revenir));
