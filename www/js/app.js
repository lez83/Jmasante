  ...
  const carte = (pl, close) => {
    ...
    <div class="plcard ${close?"ok":"on"}" data-pl="${pl.id}">
      ...
      <div class="rowb" style="gap:5px;margin-top:8px">
        <button class="btn btn-ghost sm" data-plsch="${pl.id}" style="flex:0 0 auto" title="Voir sur le schéma">🧍</button>
...
function sheetPlaieSchema(pid, plid, apres){
  ...
  const dessine = () => {
    ...
    $("#sheet").innerHTML = `
      ...
      <div class="chips" style="margin-bottom:8px">
        <button class="chip ${vue==="face"?"on":""}" data-plv="face">Face</button>
        <button class="chip ${vue==="dos"?"on":""}" data-plv="dos">Dos</button>
    ...
    $$("#sheet [data-zone]").forEach(z => z.onclick = () => {
      const n = z.dataset.zone; if (!n || n.startsWith("__")) return;
