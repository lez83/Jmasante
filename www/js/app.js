...
/* Thème de jour / thème de nuit — v1.0.62 (S.themeAuto) */
function themeEffectif(){
  const a = S.themeAuto;
  if (!a || !a.on) return S.theme;
  const now = new Date(), m = now.getHours()*60 + now.getMinutes();
  const hm = s => { const [h,mi] = String(s||"0:0").split(":").map(Number); return (h||0)*60 + (mi||0); };
  const j = hm(a.hJour || "07:00"), n = hm(a.hNuit || "19:30");
  const jour = j <= n ? (m >= j && m < n) : !(m >= n && m < j);
  return jour ? (a.jour || S.theme) : (a.nuit || S.theme);
}
let _themeTimer = null;
function applyTheme(){
  const t0 = themeEffectif();
  const t = APP_THEMES[t0] ? t0 : "original";
  document.documentElement.dataset.themeActif = t;
  if (S.themeAuto && S.themeAuto.on && !_themeTimer)
    _themeTimer = setInterval(() => { try { if (document.documentElement.dataset.themeActif !== themeEffectif()) applyTheme(); } catch(e){} }, 60000);
  ...
  // Cartes patient : pastille (défaut) ou bande latérale — v1.0.58
  document.documentElement.dataset.cards = S.cardStyle === "bande" ? "bande" : "classique";
  document.documentElement.dataset.dot = ["moyenne","grande"].includes(S.dotSize) ? S.dotSize : "petite";
  // Couleurs et motifs des statuts — v1.0.60
  try { if (typeof appliquerCouleurs === "function") appliquerCouleurs(); } catch(e){ console.error("couleurs:", e); }
  // Personnalisation — v1.0.62
  try { if (typeof appliquerZoom === "function"){ appliquerZoom(); appliquerModeZoom(); renderToolbar(); } } catch(e){ console.error("perso:", e); }
}
...
const _sqlKeys = async () => {
  const r = await _sql.query({ database:SQLDB, statement:"SELECT k FROM kv;", values:[] });
  return (r.values || []).map(x => x.k);
};
const _backKeys = () => _sql ? _sqlKeys() : _rawKeys();
...
/* Clé HISTORIQUE (v1 : sel fixe « jmsante_v1 », 100 000 itérations).
   Ne sert plus qu'à LIRE ce qui a été écrit avant la v1.0.59, puis à
   rien une fois la migration faite. Dérivée seulement si besoin.
   ⚠️ S est null au premier appel (lecture de l'état) : le code de
   verrouillage n'est donc jamais entré dans cette clé en pratique. */
...
/* ============================================================
   CLÉ LOCALE v2 — v1.0.59 (lot 3)
   Sel ALÉATOIRE propre à l'installation + 600 000 itérations
   (recommandation OWASP pour PBKDF2-SHA256). Le sel n'est pas un
   secret : il vit en clair dans « __kdf__ », à côté de « __secret__ ».
   Chaque bloc chiffré porte sa version (k:2) : lecture possible
   avec l'une ou l'autre clé, migration interruptible sans risque.
   Le secret lui-même ne change pas (il sert aussi à SQLCipher).
============================================================ */
const KDF_ITER = 600000;
let _encKey2 = null;
async function ensureEncKey2(){
  if (_encKey2) return _encKey2;
  try {
    const secret = await _getOrCreateSecret();
    let kdf = null;
    try { kdf = await _rawGet("__kdf__"); } catch(e){}   // ⚠️ comme __secret__ : toujours IndexedDB, quel que soit le backend
    if (!kdf || !kdf.salt){
      kdf = { v:2, salt:_b64(crypto.getRandomValues(new Uint8Array(16))), it:KDF_ITER };
      await _rawSet("__kdf__", kdf);
    }
    const m = await crypto.subtle.importKey("raw", new TextEncoder().encode(secret), "PBKDF2", false, ["deriveKey"]);
    _encKey2 = await crypto.subtle.deriveKey(
      { name:"PBKDF2", salt:_deb64(kdf.salt), iterations:kdf.it || KDF_ITER, hash:"SHA-256" },
      m, { name:"AES-GCM", length:256 }, false, ["encrypt","decrypt"]);
  } catch(e){ console.error("clé v2:", e); _encKey2 = null; }
  return _encKey2;
}
...
async function encryptState(obj){
  /* Toujours la clé v2 ; repli sur l'ancienne seulement si la v2 est
     impossible (jamais en clair tant qu'une clé existe). */
  const k2 = await ensureEncKey2();
  const key = k2 || await ensureEncKey();
  if (!key) return obj; // pas de chiffrement sans clé
  const plain = new TextEncoder().encode(JSON.stringify(obj));
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const cipher = await crypto.subtle.encrypt({name:"AES-GCM",iv}, key, plain);
  // Stocker comme { _enc: true, iv: base64, data: base64, k: 2 }
  const out = { _enc:true, iv:_b64(iv), data:_b64(new Uint8Array(cipher)) };
  if (k2) out.k = 2;
  return out;
}
...
async function _decAvec(key, stored){
  if (!key) return undefined;
  try {
    const plain = await crypto.subtle.decrypt({ name:"AES-GCM", iv:_deb64(stored.iv) }, key, _deb64(stored.data));
    return JSON.parse(new TextDecoder().decode(plain));
  } catch(e){ return undefined; }
}
async function decryptState(stored){
  if (!stored || !stored._enc) return stored; // pas chiffré
  /* La version portée par le bloc désigne la clé ; l'autre est tentée
     en secours (bloc écrit pendant une migration interrompue, etc.). */
  const ordre = stored.k === 2 ? [ensureEncKey2, ensureEncKey] : [ensureEncKey, ensureEncKey2];
  for (const f of ordre){
    const r = await _decAvec(await f(), stored);
    if (r !== undefined) return r;
  }
  console.warn("Déchiffrement échoué (aucune clé ne correspond)");
  return null;
}

/* Migration vers la clé v2, en tâche de fond après le démarrage.
   Bloc par bloc : chacun est relu, vérifié, puis réécrit. Une
   interruption (app fermée) ne perd rien — on reprend au lancement
   suivant. Les documents encore en clair (antérieurs au chiffrement)
   sont chiffrés au passage. S.kdfMigre marque la fin. */
let _migrEnCours = false;
async function migrerCleLocale(){
  if (_migrEnCours || !S || S.kdfMigre) return;
  if (!(await ensureEncKey2())) return;
  _migrEnCours = true;
  let n = 0, ko = 0;
  try {
    const cles = (await _backKeys()).filter(k => _aChiffrer(k) && k !== "state");
    for (const k of cles){
      const v = await _backGet(k);
      if (!v || (v._enc && v.k === 2)) continue;
      const clair = v._enc ? await decryptState(v) : v;
      if (clair === null || clair === undefined){ ko++; continue; }   // illisible : on n'y touche pas
      await _backSet(k, await encryptState(clair));
      n++;
      if (n % 5 === 0) await new Promise(r => setTimeout(r, 30));      // laisse respirer l'interface
    }
    if (!ko){
      S.kdfMigre = true;
      save(true);                       // réécrit aussi l'état avec la clé v2
    } else if (typeof logIncident === "function"){
      logIncident("migration", ko + " document(s) illisible(s) laissé(s) tel(s) quel(s)", null);
    }
    if (n) console.log("Clé locale v2 : " + n + " bloc(s) migré(s)");
  } catch(e){
    console.error("migration clé:", e);
    if (typeof logIncident === "function") logIncident("migration", "Migration de la clé locale interrompue", e);
  } finally { _migrEnCours = false; }
}
...
/* Longueur attendue : 6 chiffres pour tout NOUVEAU code (v1.0.58) ; un
   code à 4 chiffres créé avant reste valable (S.pinLen absent = 4). */
const pinLongueur = () => pinMode === "set" ? 6 : (S.pinLen || 4);
function showLock(mode){
  pinMode = mode; pinBuf = ""; pinFirst = "";
  $("#lockmsg").textContent = mode === "set" ? "Choisis un code à 6 chiffres" : "Saisis ton code pour déverrouiller";
  { const dd = $("#lockdots"); if (dd) dd.innerHTML = '<i class="d"></i>'.repeat(pinLongueur()); }
  drawDots();
  /* « Code oublié ? » : seulement pour déverrouiller */
  { let lk = document.getElementById("lock-oubli");
    if (!lk){ lk = document.createElement("button"); lk.id = "lock-oubli"; lk.className = "lock-oubli";
      lk.textContent = "Code oublié ?"; $("#lock").appendChild(lk); }
    lk.style.display = mode === "unlock" ? "" : "none";
    lk.onclick = () => { if (typeof codeOublie === "function") codeOublie(); }; }
  const pad = $("#pinpad");
...
async function pinKey(k){
  ...
  else if (pinBuf.length < pinLongueur()) pinBuf += k;
  drawDots();
  if (pinBuf.length !== pinLongueur()) return;
  const code = pinBuf;
  if (pinMode === "unlock"){
    ...
    if (!pinFirst){
      ...
      S.pin = await pinHash(code); S.pinLen = 6; save();
      $("#lock").classList.remove("on");
      toast("Code activé 🔒");
      if (typeof apresCreationCode === "function") await apresCreationCode();
      if (window._pinDepuisReglages){ window._pinDepuisReglages = false; sheetTours(); }
    } else {
      toast("Les codes ne correspondent pas", "danger");
      pinFirst = ""; pinBuf = ""; drawDots();
      $("#lockmsg").textContent = "Choisis un code à 6 chiffres";
    }
  }
}
...
async function exportBackup(mode, tourFiltre){
  ...
  /* Une tournée envoyée à un confrère passe par l'écran de protection,
     qui a son propre avertissement : pas de double message. */
  if (!(tourFiltre && mode === "share") && !S.bkAvertiVu && typeof askDialog === "function"){
  ...
  if (tourFiltre && mode === "share" && typeof sheetProtectionEnvoi === "function"){
    return sheetProtectionEnvoi(tourFiltre, { json,
      base: "JMSante_" + tourFiltre.replace(/[^\w-]+/g, "-") + "_" + todayISO(),
      titre: "Tournée JM@Santé", texte: "Dossiers de la tournée " + tourFiltre + " — " + (typeof whoami === "function" ? whoami() : "") });
  }
  const cap = window.Capacitor;
...
function vitalsHtml(c, al, thr, p){
  const _vThr = thr;
  /* v1.0.62 : ordre et constantes masquées ; carte « sans constantes » →
     seules les valeurs hors seuil restent (signal de vigilance). */
  const _seulesAlertes = typeof carteMontre === "function" && !carteMontre("consts");
  /* v1.0.58 : sens du dépassement ↑/↓ (rouge / bleu), dans les deux modes */
  const parts=[]; const push=(k,lbl,unit)=>{ if(c&&c[k]){ const b=isBad(k,al);
    const sn = b && typeof sensConst==="function" ? sensConst(k, c[k], _vThr) : "";
    parts.push(`<b class="${b?"bad":""} ${sn}">${lbl} ${esc(c[k])}${unit||""}${sn==="hi"?" ↑":sn==="lo"?" ↓":""}</b>`); } };
  const _lbl = { ta:["TA"], temp:["T°","°"], sat:["Sat","%"], puls:["♥"], glyc:["Gly"], douleur:["EVA"] };
  const _ord = typeof ordreConst === "function" ? ordreConst() : ["ta","temp","sat","puls","glyc","douleur"];
  _ord.forEach(k => {
    const bad = c && c[k] && isBad(k, al);
    if (_seulesAlertes && !bad) return;
    if (!bad && typeof constVisible === "function" && !constVisible(k, p)) return;
    push(k, _lbl[k][0], _lbl[k][1]);
  });
...
function render(){
  ...
  const cardOf = p => {
    ...
    const _tel = (p.tel||{}).mobile || (p.tel||{}).fixe
       || medecinsDe(p).some(m=>m.tel) || entourageDe(p).some(e=>e.tel)
       || Object.values(p.contacts||{}).some(c=>c&&c.tel);
    const _act = (S.cardStyle === "bande" && !open && (_tel || adresseComplete(p)))
      ? `<div class="pc-act">${_tel ? `<button class="pc-ic" data-cann="${p.id}" title="Appeler">📞</button>` : ""}${
          adresseComplete(p) ? `<button class="pc-ic" data-cgps="${p.id}" title="Itinéraire">📍</button>` : ""}</div>` : "";
    return `<div class="pcard st-${s} ${s==="alert"?"warn":""} ${s==="absent"?"absent":""} ${s==="novisit"?"novisit":""} ${s==="done"&&!open?"seen":""} ${open?"open":""}" data-id="${p.id}">
      <span class="st ${s==="absent"?"todo":s}"></span>
      <button style="text-align:left" data-toggle="${p.id}">
        <div class="nm">${esc(p.prenom)} ${esc(p.nom.replace("Demo-","").toUpperCase())}${carteMontre("age") ? ` <span class="age">${ageOf(p.dob)??"—"} ans</span>` : ""}</div>
        <div class="vitals">${vitalsHtml(c, al, p.thresholds, p)}</div>
  ...
  (S.drafts[p.id].at ? " le " + fmtFR(new Date(S.drafts[p.id].at).toISOString().slice(0,10)) : "")} ▾</button>` : ""}
          ...
          ${carteMontre("badges") && p.docs.length ? `<span class="mini blue">📎 ${p.docs.length}</span>` : ""}
          ${carteMontre("badges") && bilansPending(p).length ? `<span class="mini blue">🧪 ${bilansPending(p).length}</span>` : ""}
          ${carteMontre("badges") && raps.length ? `<span class="mini amber">📌 ${raps.length}</span>` : ""}
          ${(() => {
            const t = (carteMontre("tags") && typeof annivTexte === "function") ? annivTexte(p) : "";
          ...
          ${(p.tags||[]).map(t=>{
            ...
            if (!carteMontre("tags") && t !== "prioritaire") return "";   // prioritaire : toujours visible
        ...
        <div class="lastseen" ${carteMontre("dernier") ? "" : 'style="display:none"'}>${lv ? (lv.date===workDate() ? (isToday()?"vu aujourd'hui à ":"vu ce jour-là à ")+esc(lv.at) : "dernier passage : "+esc(fmtFR(lv.date))+" "+esc(lv.at)) : "jamais vu"}</div>
      </button>
      ${_act}
      ${open ? inlineForm(p) : ""}
    </div>`;
  ...
  /* Actions rapides des cartes à bande latérale (v1.0.58) */
  $$("[data-cann]").forEach(b => b.onclick = e => { e.stopPropagation(); const pp = getP(b.dataset.cann); if (pp) sheetAnnuaire(pp); });
  $$("[data-cgps]").forEach(b => b.onclick = e => { e.stopPropagation(); const pp = getP(b.dataset.cgps);
    if (pp) window.open(`geo:0,0?q=${encodeURIComponent(adresseComplete(pp)||pp.address||"")}`, "_system"); });
  try { renderLegende(); } catch(e){ console.error("legende:", e); }
...
function inlineForm(p){
  ...
  return `<div class="inline" data-form="${p.id}">
    ...
    ${S.constStyle === "tableau" ? vitalTilesHtml(p) : ""}
    ...
    <div class="rowbox bl" style="display:block;margin-bottom:12px">
      <div class="cgrid">
        <div data-ck="ta" style="${cstStyle("ta", p)}"><label>TA</label>
        ...
        <div data-ck="temp" style="${cstStyle("temp", p)}"><label>T° °C</label><input data-c="temp" inputmode="decimal" placeholder="${esc(gh(_lc.temp,"36.8"))}"></div>
        <div data-ck="sat" style="${cstStyle("sat", p)}"><label>Sat %</label><input data-c="sat" inputmode="numeric" placeholder="${esc(gh(_lc.sat,"97"))}"></div>
        <div data-ck="puls" style="${cstStyle("puls", p)}"><label>Pouls</label><input data-c="puls" inputmode="numeric" placeholder="${esc(gh(_lc.puls,"72"))}"></div>
        <div data-ck="glyc" style="${cstStyle("glyc", p)}"><label>Gly g/L</label><input data-c="glyc" inputmode="decimal" placeholder="${esc(gh(_lc.glyc,"1.10"))}"></div>
        <div data-ck="douleur" style="${cstStyle("douleur", p)}"><label>EVA /10</label><input data-c="douleur" inputmode="numeric" placeholder="${esc(gh(_lc.douleur,"0"))}"></div>
      </div>
      <div class="selrow">
        <label>Selles</label>
        <div class="selbtns">
...
function _bindUnForm(p, f){
  ...
  try { brancherTuilesSaisie(f); } catch(e){ console.error("tuiles:", e); }
...
function sheetTours(){
  ...
  const tiles = `
    <div class="mgrid">
      ...
      <button class="mtile" data-sec="app"><span class="mt-ic">⚙️</span><span class="mt-l">Application</span>
        <span class="mt-s">Sécurité · données · aide</span></button>
      <button class="mtile mt-large" data-sec="perso"><span class="mt-ic">🎨</span><span class="mt-l">Personnaliser</span>
        <span class="mt-s">Thème · couleurs · cartes · texte et zoom</span></button>
    </div>
  ...
  const list = `
    ...
    <div class="mgroup">
      ...
      ${row("collegues","🤝","Collègues appairés", (S.collegues||[]).length)}
    </div>
    ...
    <div class="mgroup">
      ${row("perso","🎨","Personnaliser l'affichage", esc((APP_THEMES[S.theme]||{}).lbl||""))}
...
function menuGo(sec){
  switch(sec){
    ...
    case "collegues": sheetCollegues(); break;
    ...
    case "theme":    sheetPersoZone("couleurs"); break;
    case "perso":    sheetPersonnaliser(); break;
...
async function bindMenuHandlers(){
  ...
  $$("#tourlist [data-tenv]").forEach(b => b.onclick = () => ensureIdentity(() => sheetProtectionEnvoi(b.dataset.tenv, null)));
  ...
  /* Affichage des constantes / cartes patient / aides — v1.0.58 */
  $$("#cstpick [data-cst]").forEach(b => b.onclick = () => {
    S.constStyle = b.dataset.cst; save();
    $$("#cstpick [data-cst]").forEach(x=>x.classList.toggle("on",x===b));
    render();
    if (S.constStyle === "tableau") aideUne("tuiles", { ic:"📊", titre:"Constantes en tableau de bord",
      sub:"Ouvre la carte d'un patient : chaque constante a sa tuile, avec sa <b>dernière valeur</b> et <b>quand</b> elle a été prise.<br><br>" +
          "<b>↑ HAUT</b> en rouge, <b>↓ BAS</b> en bleu : la valeur dépasse un seuil que tu as fixé. L'app compare, elle n'interprète pas.<br><br>" +
          "La date passe en <b>ambre</b> au-delà de 24 h : la valeur est ancienne. Une constante jamais mesurée reste en place, estompée. <b>Touche une tuile</b> : dans la carte ouverte, elle t'amène au champ de saisie de cette constante, clavier ouvert ; dans la fiche, elle ouvre les 📈 Courbes." });
  });
  $$("#cardpick [data-card]").forEach(b => b.onclick = () => {
    S.cardStyle = b.dataset.card; save(); applyTheme();
    $$("#cardpick [data-card]").forEach(x=>x.classList.toggle("on",x===b));
    render(); sheetAppPanel();
    if (S.cardStyle === "bande") aideUne("bande", { ic:"▌", titre:"Cartes à bande latérale",
      sub:"La couleur du bord gauche donne le statut du patient d'un coup d'œil : " +
          "<b>ambre</b> à voir, <b>vert</b> vu, <b>rouge</b> vigilance, <b>gris</b> absent ou sans passage.<br><br>" +
          "📞 ouvre ses numéros, 📍 lance l'itinéraire. La légende reste au-dessus de la liste (ⓘ)." });
  });
  { const gc = $("#go-couleurs"); if (gc) gc.onclick = () => sheetCouleurs(); }
  { const gp = $("#go-perso"); if (gp) gp.onclick = () => sheetPersonnaliser(); }
  $$("#dotpick [data-dot]").forEach(b => b.onclick = () => {
    S.dotSize = b.dataset.dot; save(); applyTheme();
    $$("#dotpick [data-dot]").forEach(x=>x.classList.toggle("on",x===b)); render(); });
  { const ar = $("#aides-reset"); if (ar) ar.onclick = () => { S.aides = {}; save(); render(); toast("Les aides réapparaîtront 💡"); }; }
  ...
  if (pinOn) pinOn.onclick = async () => { closeSheet(); window._pinDepuisReglages = true; showLock("set"); };
  /* Code de secours : créer (codes antérieurs à v1.0.58) ou régénérer */
  { const r = $("#pin-rescue"); if (r) r.onclick = async () => {
      if (S.pinRescue && !await askDialog({ ic:"🔑", titre:"Régénérer le code de secours ?",
          sub:"L'ancien code de secours ne fonctionnera plus.", oui:"Régénérer" })) return;
      await creerCodeSecours(); sheetTours(); }; }
  if (pinOff) pinOff.onclick = async () => {
    if (!await askDialog({ ic:"❓", titre:"Désactiver le code de verrouillage ?" })) return;
    S.pin = null; S.pinLen = null; S.pinRescue = null; S.bioLock = false; save(); toast("Code désactivé"); sheetTours();
  };
  ...
  { const c = $("#sync-coll"); if (c) c.onclick = () => sheetCollegues(); }
...
<p class="small" style="margin-bottom:8px"><b>📊 Affichage des constantes</b> (même endroit) : <b>Classique</b> garde la ligne actuelle ; <b>Tableau de bord</b> montre une tuile par constante dans la carte ouverte et dans <b>✏️ Fiche → 💉 Soins</b>. Chaque tuile donne la <b>dernière valeur connue</b> et <b>sa date</b> (en ambre si elle a plus de 24 h). <b>↑ HAUT</b> (rouge) ou <b>↓ BAS</b> (bleu) : la valeur dépasse un seuil que tu as fixé — l'app compare, elle n'interprète pas. Une constante jamais mesurée reste en place, estompée. Sur le Moniteur, la ligne compacte affiche aussi ↑ ou ↓.</p>
<p class="small" style="margin-bottom:8px"><b>🪪 Cartes patient</b> : <b>Pastille</b> (petit rond en haut à droite) ou <b>Bande latérale</b> (tout le bord gauche coloré, avec 📞 pour appeler et 📍 pour l'itinéraire directement sur la carte). Les couleurs sont les mêmes : <b>ambre</b> à voir · <b>vert</b> vu · <b>rouge</b> vigilance · <b>gris</b> absent ou sans passage. En mode Pastille, trois <b>tailles</b> : Petite, Moyenne ou Grande (plus lisible de loin, et les formes ✓ ▲! bien nettes). Quand la carte est <b>dépliée</b>, la bande disparaît pour laisser toute la largeur au formulaire. La <b>légende</b> s'affiche au-dessus de la liste ; « Ne plus afficher » la replie en <b>ⓘ Légende</b>, qui la rouvre d'un toucher.</p>
<p class="small" style="margin-bottom:8px"><b>🎨 Personnaliser</b> (🦗 → tuile 🎨, ou ⚙️ Application → Personnaliser l\'affichage) — tout ce qui change l\'apparence est rangé par zone, avec un <b>aperçu en direct</b> en haut de chaque écran : 🖥 Moniteur (présentation, barre d\'outils, écran d\'ouverture) · 🪪 Cartes patient · 📊 Constantes · 🎨 Couleurs et thèmes · 🔤 Texte et zoom · 📋 Relève · 📤 Partager mes réglages. <b>↺ Réinitialiser l\'affichage</b> remet tout à l\'origine, sans toucher à tes données ni à ton thème.</p>
<details class="small" style="margin-bottom:8px"><summary><b>En détail : Personnaliser</b></summary>
<p style="margin:6px 0"><b>🔤 Texte et zoom</b> — trois tailles (Normale, Grande, Très grande ; à partir de 130 %, les cartes passent sur une colonne). <b>Zoom à deux doigts</b>, au choix : <b>Intelligent</b> (conseillé) — écarte deux doigts n\'importe où, toute l\'app grossit en se réorganisant dans la largeur de l\'écran, un badge montre le niveau et ↺ remet à 100 %, le niveau est gardé ; <b>Natif</b> — le zoom classique du téléphone, comme une page web (défilement de côté possible, le bouton du bas peut sortir de l\'écran le temps du zoom) ; <b>Désactivé</b> — aucun zoom à deux doigts. Sur ordinateur : Ctrl + molette. Les <b>photos et documents</b> ont leur propre zoom, jusqu\'à ×5 : pince pour zoomer, glisse pour te déplacer, double-touche pour basculer.</p>
<p style="margin:6px 0"><b>📊 Constantes</b> — coche celles que tu utilises et règle leur ordre (▲▼). Une constante masquée disparaît de la saisie, des tuiles et de la ligne du Moniteur, <b>jamais de l\'historique, des courbes ni de la relève</b> ; une valeur hors seuil reste toujours affichée. Pour un patient précis (la glycémie d\'un diabétique), force-la dans <b>✏️ Fiche → 💉 Soins</b>.</p>
<p style="margin:6px 0"><b>🪪 Cartes patient</b> — choisis ce qui s\'affiche sur une carte repliée (âge, constantes, badges 📎🧪📌, étiquettes, dernier passage). Les signaux de vigilance restent toujours visibles : valeur hors seuil, jours sans selle, patient prioritaire, absence.</p>
<p style="margin:6px 0"><b>🖥 Moniteur</b> — jusqu\'à 5 raccourcis dans la barre d\'outils, dans l\'ordre voulu, parmi Chercher, Déroulé, Rappels, Patient, Relève, Note, Route, Synchro ; la 🦗 Cigale reste toujours en premier. <b>À l\'ouverture</b> : la dernière tournée utilisée ou toujours la même, et le moment (selon l\'heure, Journée, Matin, Soir).</p>
<p style="margin:6px 0"><b>🎨 Couleurs et thèmes</b> — <b>thème de jour et thème de nuit</b> : l\'app bascule seule aux heures choisies (par ex. Bloc à 7 h, Hôpital de nuit à 19 h 30).</p>
<p style="margin:6px 0"><b>📋 Relève</b> — format et contenu présélectionnés, patients en vigilance en premier, rappels au début ou à la fin, et une <b>signature</b> ajoutée en bas de chaque relève.</p>
<p style="margin:6px 0"><b>📤 Partager mes réglages</b> — un fichier à envoyer à un collègue : il l\'ouvre avec JM@Santé et choisit de les appliquer. Liste fermée : <b>jamais</b> de patient, de tournée, de mot de passe, de collègue ni d\'identité. Il peut revenir aux siens d\'un toucher (↺).</p></details>
<p class="small" style="margin-bottom:8px"><b>🎨 Couleurs des statuts</b> (🦗 → Application) : choisis une <b>palette</b> — JM@Santé (suit le thème), Feu tricolore, Contraste fort (plein soleil), Daltonisme — puis retouche une couleur si tu veux, avec un <b>aperçu</b> en direct. Un avertissement s\'affiche si deux statuts deviennent trop proches. Les couleurs valent partout : pastilles, bande latérale, légende, compteurs d\'avancement et valeurs hors seuil. <b>Motifs et formes (aide daltonisme)</b> : chaque statut reçoit en plus un motif sur la bande et dans la légende (plein, rayures, points, quadrillage, au choix), et une forme sur la pastille — <b>●</b> à voir, <b>✓</b> vu, <b>▲!</b> vigilance, <b>▬</b> absent. On reconnaît ainsi les statuts sans distinguer les couleurs. La palette Daltonisme active les motifs d\'office.</p>
<p class="small" style="margin-bottom:8px"><b>💡 Aides</b> : à la première utilisation d'une fonction, une bulle l'explique. Coche « Ne plus afficher » pour ne plus la voir. <b>🦗 → Application → Revoir les aides et la légende</b> les fait toutes réapparaître (pratique pour montrer l'app à un collègue).</p>
<p class="small" style="margin-bottom:8px"><b>🔒 Code de verrouillage et code de secours</b> : tant qu'aucun code n'existe, l'app te le propose aux 5 premiers lancements. Les nouveaux codes ont <b>6 chiffres</b> (un code à 4 chiffres créé avant reste valable). À la création, l'app te donne un <b>code de secours</b> du type <b>K7M2-9QXT-4HB8</b> : note-le <b>ailleurs que dans le téléphone</b>, il n'est affiché qu'une fois. Sur Android, elle te propose ensuite l'empreinte. <b>Code oublié ?</b> (sous le clavier) : ton code de secours rouvre l'app et te fait choisir un nouveau code ; sans lui, la seule issue est de <b>tout effacer</b> puis de réimporter ta dernière sauvegarde. Tes données, elles, ne sont jamais touchées par un code oublié. Un code déjà créé sans code de secours : <b>🦗 → Sécurité → Créer mon code de secours</b>.</p>
<p class="small" style="margin-bottom:8px"><b>🔐 Chiffrement sur l'appareil</b> : tes dossiers et documents sont chiffrés dans le téléphone avec une clé propre à cette installation. Depuis la v1.0.59, cette clé est renforcée (sel unique à ton appareil, 600 000 itérations). Le passage se fait <b>tout seul, en arrière-plan</b>, quelques secondes après l'ouverture : rien à faire, et une interruption (app fermée) ne perd rien — il reprend au lancement suivant.</p>
<p class="small" style="margin-bottom:8px"><b>🔐 Protéger les envois à tes collègues</b> — l'essentiel : chaque collègue d'une tournée reçoit la synchro ou le premier échange de l'une de trois façons. <b>🤝 Appairé</b> : chiffré pour lui seul, il n'a rien à taper. <b>🔑 Mot de passe</b> : le mot de passe de la tournée, donné une fois. <b>📤 Libre</b> : comme avant, si tu passes par une messagerie chiffrée. Le réglage se fait dans <b>🗺️ Mes tournées → 🔐</b> et reste modifiable à chaque envoi.</p>
<details class="small" style="margin-bottom:8px"><summary><b>En détail : envoi protégé</b></summary><p style="margin:6px 0">Tu peux <b>mélanger</b> dans une même tournée : Pierre appairé, Sandra au mot de passe, Yann en libre. L'app prépare au plus <b>deux fichiers</b> : <b>…_PROTEGE.json</b>, qui s'ouvre pour tous les appairés ET avec le mot de passe, et <b>…_LIBRE.json</b>, lisible par qui l'ouvre. Le nom du fichier te dit toujours lequel tu envoies.</p><p style="margin:6px 0"><b>Le mot de passe de la tournée</b> est le même à chaque envoi : tu le changes quand tu veux (✏️ Changer), et <b>toujours quand quelqu'un quitte le cabinet</b>. L'app te le rappelle au bout de 6 mois. 🎲 en propose un facile à dicter. Donne-le <b>par un autre canal</b> que le fichier (SMS, de vive voix) — jamais dans le même fil WhatsApp. Ton collègue peut le faire retenir par son app : il ne le tape qu'une fois.</p><p style="margin:6px 0"><b>Mot de passe perdu ?</b> Change-le, donne le nouveau, renvoie la relève. Un fichier ne s'ouvre jamais sans le bon mot de passe : personne ne peut le récupérer.</p><p style="margin:6px 0"><b>Envoi libre</b> : un avertissement le rappelle avant chaque envoi. Si tu utilises une messagerie chiffrée santé, coche « Ne plus afficher » ; « Revoir les aides » le fait revenir.</p><p style="margin:6px 0"><b>À la réception</b>, l'app essaie d'abord les appairages et les mots de passe retenus, et ne demande le mot de passe qu'en dernier recours. Mot de passe erroné : <b>rien n'est importé</b>, tes données restent intactes. Seuls les fichiers d'échange (.json) sont concernés : les relèves PDF, Word ou HTML restent lisibles par un collègue qui n'a pas l'app. Un collègue sur une ancienne version ne peut pas ouvrir un fichier protégé : il doit mettre JM@Santé à jour.</p></details>
<p class="small" style="margin-bottom:8px"><b>🤝 Appairer un collègue</b> (🦗 → Partage → Collègues appairés) — une fois par collègue, côte à côte. <b>🛡 Sécurisé</b> (conseillé) : vous scannez chacun le code de l'autre ; ces codes ne contiennent rien de secret. <b>⚡ Rapide</b> : un seul scan, mais le code affiché <b>est la clé</b> — ne le laisse pas à l'écran, ne le photographie pas. À la fin, un <b>code de contrôle</b> à 4 chiffres doit être <b>identique</b> sur les deux téléphones.</p>
<details class="small" style="margin-bottom:8px"><summary><b>En détail : appairage</b></summary><p style="margin:6px 0"><b>Pas de caméra</b> (PC) ou pas côte à côte : 📋 Copier mon code, puis ton collègue le colle (⌨️ Coller son code), par exemple pendant une visio. Sur Android, la première fois, autorise l'accès à la caméra puis touche à nouveau 📷 Scanner.</p><p style="margin:6px 0"><b>Un collègue a perdu ou changé de téléphone ?</b> <b>Oublie-le</b> : son ancien appareil ne pourra plus ouvrir tes envois, et il passe automatiquement en 🔑 mot de passe dans tes tournées. Envoie-lui ainsi sur son téléphone de secours, puis refaites l'appairage avec le nouveau. Les mots de passe que ton app a retenus pour des fichiers reçus se gèrent au même endroit.</p></details>
  ...
  menuSheet("🗺️ Mes tournées", `
    <div id="tourlist">${S.tours.map(t => `
      <div class="rap"><span class="ric">🗺</span>
        ...
        <button class="btn btn-ghost btn-sm" data-tenv="${esc(t)}" style="flex:none" title="Protection des envois">🔐</button>
...
function sheetSharePanel(){
  menuSheet("🔄 Partage & synchronisation", `
    ...
    <button class="btn btn-ghost" id="sync-coll" style="margin-bottom:10px;width:100%">🤝 Collègues appairés (${(S.collegues||[]).length})</button>
    <p class="small muted" style="margin-bottom:10px">Le réglage d'envoi (appairé / mot de passe / libre) se fait par tournée : 🗺️ Mes tournées → 🔐.</p>
...
function sheetDataPanel(){
  ...
  menuSheet("💾 Mes données", `
    ...
    ${S.pin ? `<button class="btn btn-ghost" id="pin-rescue" style="width:100%;margin-bottom:8px">🔑 ${S.pinRescue ? "Régénérer mon code de secours" : "Créer mon code de secours"}</button>` : ""}
    <p class="small muted" style="margin-bottom:16px">Code à 6 chiffres demandé à l'ouverture (un code à 4 chiffres créé avant reste valable). ${S.pin && !S.pinRescue ? "<b>Tu n'as pas encore de code de secours</b> : sans lui, un code oublié oblige à tout effacer." : "Le code de secours ouvre l'app si tu oublies ton code."}</p>
    <div style="height:1px;background:var(--border);margin:16px 0"></div>
...
function sheetAppPanel(){
  menuSheet("⚙️ Application", `
    <button class="btn btn-primary" id="go-perso" style="width:100%;margin-bottom:6px">🎨 Personnaliser l'affichage</button>
    <p class="small muted" style="margin-bottom:16px">Thème, couleurs, cartes, constantes, texte et zoom, barre d'outils — avec aperçu en direct.</p>
    <div style="height:1px;background:var(--border);margin:16px 0"></div>
...
function sheetTheme(){    sheetPersoZone("couleurs"); }
...
function sheetPatient(p){
  ...
  openSheet(`
    ...
    ${p && S.constStyle === "tableau" && typeof vitalTilesHtml === "function"
      ? `<div class="field"><span class="lab">Dernières constantes</span>${vitalTilesHtml(p)}</div>` : ""}
    ${p && (S.constsMasquees||[]).length ? `<div class="field"><span class="lab">Constantes masquées — toujours afficher pour ce patient</span>
      <div class="chips" id="f-cforce">${(S.constsMasquees||[]).map(k => `<button type="button" class="chip ${(p.constsForcees||[]).includes(k)?"on":""}" data-cforce="${k}">${esc(CONST_LBL[k]||k)}</button>`).join("")}</div>
      <p class="small muted" style="margin-top:4px">Par exemple la glycémie chez un patient diabétique, même si tu l'as masquée pour la tournée.</p></div>` : ""}
  ...
  try { if (p) brancherTuilesFiche(p); } catch(e){ console.error("tuiles fiche:", e); }
  $$("#sheet [data-cforce]").forEach(b => b.onclick = () => {
    const f = new Set(p.constsForcees || []); const k = b.dataset.cforce;
    f.has(k) ? f.delete(k) : f.add(k); p.constsForcees = [...f];
    b.classList.toggle("on", f.has(k)); save(); render(); });
...
function sheetReleve(){
  ...
  let mode = ["full","events","select"].includes((S.releveDefaut||{}).mode) ? S.releveDefaut.mode : "full", layout = "structure";
  /* Relève par défaut — v1.0.62 : présélection du mode choisi dans 🎨 Personnaliser */
  setTimeout(() => { $$("#sheet .chip[data-m]").forEach(x => x.classList.toggle("on", x.dataset.m === mode)); }, 0);
...
function buildReleve({start, end, mode, withRaps, keep, pOpts, layout, anon, tour}){
  ...
  /* v1.0.62 : patients en vigilance sur la période en premier (option) */
  if ((S.releveDefaut||{}).vigilanceDabord){
    const aAl = p => p.visits.some(v => v.date>=start && v.date<=end && alertes(v.consts, p.thresholds).length) ? 1 : 0;
    pool.sort((a,b) => aAl(b) - aAl(a));
  }
  const poolIds = new Set(pool.map(p=>p.id));
  ...
  const _rd = S.releveDefaut || {};
  const _sig = _rd.signature ? "\n\u2014 " + _rd.signature + "\n" : "";
  return head + (_rd.rappelsEnFin ? "" : rapBlock) + (body || "Aucun passage sur la période.\n")
       + (_rd.rappelsEnFin ? rapBlock : "") + pied + _sig + tail;
}
...
async function showReport(text, opts, keepExtras){
  ...
  let fmt = (keepExtras && _keepFmt) ? _keepFmt : ((S.releveDefaut||{}).format || "txt");   // v1.0.62 : format par défaut
  // Un réaffichage (retour depuis 💬 Message ou ✍️ Signer) doit conserver
...
document.addEventListener("click", async e => {
  ...
  switch (a.dataset.a){
    ...
    case "route":       if (typeof shareFeuilleRoute === "function") shareFeuilleRoute(); break;
    case "sync":        ensureIdentity(() => sheetSendSync()); break;
