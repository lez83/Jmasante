...
const activeP = () => S.patients.filter(p=>!p.archived);
...
function render(){
  ...
  const F = [["all","Tous"],["todo","À voir"],["alert","⚠ Vigilance"],["done","Vus"],["absent","Absents"]];
  ...
  $("#board").innerHTML = list.map(p => {
    ...
    return `<div class="pcard ${s==="alert"?"warn":""} ${s==="absent"?"absent":""} ${s==="done"&&!open?"seen":""} ${open?"open":""}" data-id="${p.id}">
...
function _saveDraft(f, pid){
  ...
  _formDraft = { pid, soins, consts, taS, taD, note, soinNotes:{..._soinNotes} };
}
...
function showReport(text, opts){
  ...
  openSheet(`
    ...
    <div class="rowb" style="margin-top:8px;gap:8px">
      ...
      <button class="btn btn-ghost" id="rp-sig"  style="flex:1">✍️ Signer</button>
    </div>
  ...
  const buildPdf = async () => {
    ...
    doc.text("RELEVE INFIRMIERE - JM@Sante", M, 9); doc.setTextColor(0,0,0); y=19;
    ...
    for (const part of parts){
      ...
      if (!nameLine){
        lines.forEach(l=>{ const c=cl(l); if(!c||c.match(/^[-=]+$/)) return;
          ...
          doc.text(c,M,y); y+=4.5; }); doc.setTextColor(0,0,0); continue;
      }
      // Entête patient
      chk(16);
    ...
    for(let i=1;i<=pages;i++){
      ...
      doc.text("JM@Sante - "+fmtFR(todayISO())+" "+nowHM()+" | Page "+i+"/"+pages, M, pageH-6);
      doc.setTextColor(0,0,0);
    }
  ...
  const buildHtml = async () => {
    ...
    html += `<h1 id="top">📋 Relève JM@Santé — ${fmtFR(todayISO())}</h1>`;
    ...
    html += `<hr><p style="font-size:8pt;color:#999">Généré par JM@Santé le ${fmtFR(todayISO())} à ${nowHM()}</p></body></html>`;
    return html;
  };
  ...
  $("#rp-sig").onclick = () => openSignature(sig => {
    $("#rp-sig").textContent = sig ? "✍️ Signé ✓" : "✍️ Signer";
    toast(sig ? "Signature enregistrée ✓" : "Signature retirée");
  });
