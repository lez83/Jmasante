  ...
  const cardOf = p => {
    ...
    return `<div class="pcard st-${s} ${s==="alert"?"warn":""} ${s==="absent"?"absent":""} ${s==="novisit"?"novisit":""} ${s==="done"&&!open?"seen":""} ${open?"open":""}" data-id="${p.id}">
      ...
      <button style="text-align:left" data-toggle="${p.id}">
        ...
        <div class="vitals">${vitalsHtml(c, al, p.thresholds)}</div>
...
function sheetAppPanel(){
  menuSheet("⚙️ Application", `
    ...
    <span class="lab" style="display:block;margin-bottom:8px">📊 Affichage des constantes</span>
    <div class="chips" id="cstpick" style="margin-bottom:6px">
      <button class="chip ${S.constStyle!=="tableau"?"on":""}" data-cst="classique">📝 Classique</button>
      <button class="chip ${S.constStyle==="tableau"?"on":""}" data-cst="tableau">📊 Tableau de bord</button></div>
    <p class="small muted" style="margin-bottom:16px">Tableau de bord : une tuile par constante (dernière valeur, date, badge selon les seuils) dans la carte ouverte et la fiche → 💉 Soins.</p>
    <span class="lab" style="display:block;margin-bottom:8px">🪪 Cartes patient</span>
    <div class="chips" id="cardpick" style="margin-bottom:6px">
      <button class="chip ${S.cardStyle!=="bande"?"on":""}" data-card="classique">● Pastille</button>
      <button class="chip ${S.cardStyle==="bande"?"on":""}" data-card="bande">▌ Bande latérale</button></div>
    ${S.cardStyle !== "bande" ? `<div class="chips" id="dotpick" style="margin-bottom:6px"><span class="small muted" style="align-self:center;margin-right:4px">Taille :</span>
      ${[["petite","Petite"],["moyenne","Moyenne"],["grande","Grande"]].map(([k,l]) =>
        `<button class="chip ${(S.dotSize||"petite")===k?"on":""}" data-dot="${k}">${l}</button>`).join("")}</div>` : ""}
    <p class="small muted" style="margin-bottom:10px">Pastille : le petit repère en haut à droite, en trois tailles. Bande latérale : le statut colore tout le bord gauche de la carte, avec 📞 et 📍 à portée de pouce.</p>
    <button class="btn btn-ghost" id="go-couleurs" style="width:100%;margin-bottom:8px">🎨 Couleurs des statuts${(S.couleurs&&S.couleurs.motifs)?" · motifs":""}</button>
    <button class="btn btn-ghost" id="aides-reset" style="width:100%;margin-bottom:16px">💡 Revoir les aides et la légende</button>
    <div style="height:1px;background:var(--border);margin:16px 0"></div>
