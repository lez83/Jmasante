...
function sheetPECList(){
  const list = (S.patients||[]).filter(p => p.pec && !p.archived)
      ...
      ${list.length ? list.map(p=>{
        ...
        return `<div class="rap" style="align-items:center">
          <span style="flex:1">
            ...
            <div class="rs">Fin le ${fmtFR(p.pec.end)}${p.pec.motif?" · "+esc(p.pec.motif):""}</div>
...
function sheetTours(){
  ...
  openSheet(`
    ...
    <button class="btn btn-ghost" id="go-pec" style="margin-bottom:8px;width:100%">🎗️ Prises en charge terminées (${(S.patients||[]).filter(x=>x.pec&&!x.archived).length})</button>
...
<button class="btn btn-ghost" id="guide-close" style="margin-top:12px">Fermer</button>`);
    ...
    $("#guide-close").onclick = sheetTours; };
